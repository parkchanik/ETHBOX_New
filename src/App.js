...
import SendBoxForm from './components/SendBoxForm'
  ...
  onSendABox ({BoxETHValue , BoxMessageValue}) {
    ...
    this.state.web3.eth.getAccounts((error, accounts) => {
        SendABox.deployed().then((instance) => {
          ...
          //return sendabox.Contract_SendABox.sendTransaction(boxmessage , {from:accounts[0] , value:this.state.web3.toWei(boxeth , "ether") , gas: 2100000}); //web3 1.0.0 이후 towei 가안됨
          return sendabox.Contract_SendABox.sendTransaction(boxmessage , {from:accounts[0] , value:ether , gas: 2100000});
  ...
  render() {
    ...
    return (
      ...
      <Grid style={{
        ...
        <Row >
          <Col lg={7}>
          <SendBoxForm onSubmit={this.onSendABox} address={this.state.account}  currentnetwork={this.state.currentnetwork}/>
          </Col>
          <Col lg={5}>
          <Ranking uniquekey={this.uniqueId()}/>
          </Col>
        </Row>
        <Row>
